...
- Lift-over uses `bcftools +liftover` as the default engine for performance (~150× faster than `pyliftover` on a full 23andMe export — under one minute for 631K variants vs ~20 minutes). The `Liftover` Protocol abstracts this; alternative implementations (`IdentityLiftover` for native GRCh38, `PyLiftoverWrapper` — implemented as `PyLiftover` — for testing/fallback) remain available. Phase 4's imputation pipeline reuses the same `bcftools` subprocess approach. `bcftools` is in the locked tech stack; the plugin is available in distro packages of bcftools 1.19+ (Ubuntu/WSL: `sudo apt install -y bcftools`). The CLI exposes `--liftover-engine {auto,bcftools,pyliftover}` for explicit override, defaulting to `auto` (bcftools when on `$PATH`).
- Lift-over can produce non-canonical contigs regardless of engine — a canonical GRCh37 coordinate may map to e.g. `4_GL000008v2_random` on GRCh38. The normalize step re-runs `normalize_chrom` on the post-lift chromosome, drops the row when the result is `None`, and counts it in `ingestion_runs.variants_dropped_lift_to_non_canonical`. The same positive-rule filter is applied at both parse time and normalize time, so the writer's `chromosome_enum` cast never sees a non-canonical label regardless of where it originated.
