...
def _write_synthetic_vcf_without_contig_header(dest: Path, chrom_label: str) -> None:
    """Build a Beagle-style imputed VCF that omits the ##contig line.

    Triggers htslib's "Contig 'chr<N>' is not defined in the header" warning
    on cyvcf2 iteration unless the suppression context manager is in
    effect.
    """
    header = (
        "##fileformat=VCFv4.2\n"
    ...
    body = (
        f"{chrom_label}\t100\trs1\tA\tG\t.\tPASS\tDR2=0.95\tGT\t0|1\n"
        f"{chrom_label}\t200\trs2\tA\tG\t.\tPASS\tDR2=0.99\tGT\t1|1\n"
    )
    with gzip.open(dest, "wt", encoding="ascii") as out:
        out.write(header)
        out.write(body)
...
def test_import_suppresses_htslib_contig_warning(
    isolated_settings: dict[str, str],
    capfd: pytest.CaptureFixture[str],
) -> None:
    """A Beagle-style VCF (no ##contig) imports without the per-record warning.

    Uses ``capfd`` because htslib writes to the C-level stderr (fd 2),
    which ``capsys`` does not see.
    """
    init_databases()
    archive_root = isolated_settings_archive_root(isolated_settings)
    imp_id = _seed_completed_run(archive_root=archive_root)
    archive = ImputationArchive.for_run(archive_root, imp_id)
    _write_synthetic_vcf_without_contig_header(
        archive.result_dir / "chr22.dose.vcf.gz",
        "chr22",
    )
    ...
    assert "Contig 'chr22'" not in captured.err
    assert "is not defined in the header" not in captured.err


def test_dry_run_suppresses_htslib_contig_warning(
    isolated_settings: dict[str, str],
    capfd: pytest.CaptureFixture[str],
) -> None:
    """The dry-run path uses the same read helper; warning must be silenced too."""
    init_databases()
    archive_root = isolated_settings_archive_root(isolated_settings)
    imp_id = _seed_completed_run(archive_root=archive_root)
    archive = ImputationArchive.for_run(archive_root, imp_id)
