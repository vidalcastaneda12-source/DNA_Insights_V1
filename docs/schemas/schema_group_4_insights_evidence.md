...
2. **Supersession, not update.** When ClinVar reclassifies a variant, we do **not** UPDATE the insight — we INSERT a new active row, then set the old one's `is_active = FALSE` and `superseded_by = <new_id>`. History is preserved; the diff feed becomes trivial.
