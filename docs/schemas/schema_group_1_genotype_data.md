...
### chrX corrected dosage (PR 5a)

Male non-PAR chrX is stored homozygous-diploid under the R1 representation;
`corrected_dosage` maps that `2 -> 1` / `0 -> 0` to the true hemizygous copy
number, and `male_nonpar_het_anomaly` flags the biologically-impossible male
non-PAR het (stored dosage 1). PAR, female, and autosomal rows pass through
unchanged. Profile sex is derived in-SQL from the chip `sample_qc` rows (the
`profile_sex` CTE mirrors `genome.imputation.sex.profile_sex_label`); nothing is
persisted. The non-PAR predicate is the strict complement of PAR1/PAR2 (matching
`genome.par_regions.is_nonpar`), so the two telomeric slivers count as non-PAR.

This is a view-only addition (finding-029): materializing it on an existing
`genome.duckdb` is a targeted `CREATE OR REPLACE VIEW`, not a `rm -rf data/`
rebuild (PR #68 precedent).
