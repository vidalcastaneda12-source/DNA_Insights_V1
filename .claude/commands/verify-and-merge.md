...
2. **Run the protocol.** Run `scripts/verify.sh` (the always-run dev-loop tail:
   `uv sync`, `pytest`, `ruff check`, `ruff format --check`, `mypy --strict backend/src`,
   `genome docs check`). Capture each step's **exit code** — that, not stdout text, is the
   status signal.
